...
if __name__ == '__main__':
    ...
    # 假设 src.utils.split_all_categories_train_val_test 返回 (X_train, y_train, X_val, y_val, test_file_list, test_label_list)
    X_train, y_train, X_val, y_val, test_file_list, test_label_list = src.utils.split_all_categories_train_val_test(val_ratio=0.2, test_ratio=0.2, return_test_files=True)
    ...
    if load_model and os.path.exists(model_path):
        ...
        # 2. 训练模型
        model = src.model.Week1DCNN(in_channels=2, num_classes=3)
        model = src.train.train(model, X_train, y_train, X_val, y_val, batch_size=32, epochs=30, lr=2e-5, device='cuda' if torch.cuda.is_available() else 'cpu')
        # 3. 保存模型权重
    ...
    for fpath, true_label in zip(test_file_list, test_label_list):
        ...
        vote_label, prob_label = src.utils.get_file_label_from_samples(pred_labels, pred_probs)
    ...
    src.utils.plot_classification_report(y_true, y_pred, class_names=class_names)
