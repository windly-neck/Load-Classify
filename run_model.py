...
def train_and_save_models():
    for i in range(NUM_RUNS):
        X_train, y_train, X_val, y_val, _, _ = src.utils.split_all_categories_train_val_test(val_ratio=0.2, test_ratio=0, return_test_files=True, random_state=i+42)
        # 统计训练集和验证集类别分布
        ...
        model = src.model.Week1DCNN(in_channels=2, num_classes=NUM_CLASSES)
...
def vote_and_report(all_preds, npy_files):
    ...
    # 若有真实标签，可对比精度
    # src.utils.plot_classification_report(y_true, final_preds, class_names=CLASS_NAMES)
